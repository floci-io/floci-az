...
| **Azure Container Apps** | ARM path (`Microsoft.App`) | Managed environment/container app/job lifecycle (create/update/delete/list by rg + subscription), `listSecrets`, azurerm-safe read-backs (canonical enum casing, defaults filled in, secrets never echoed on GET); depends on Azure Monitor's Log Analytics workspace support for `log_analytics_workspace_id`; mocked: no Docker (container backing planned) |
