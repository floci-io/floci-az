...
class AciHandlerTest {
    ...
    @Test
    @DisplayName("DELETE answers 200 with the resource body, then 404 on GET, 204 when absent")
    void deleteThenGet404() {
        createGroup("cg-del");
        // Spec: 200 + the deleted group's body on the existing path.
        given().when().delete(BASE + "/containerGroups/cg-del" + API)
                .then().statusCode(200)
                .body("name", equalTo("cg-del"))
                .body("properties.provisioningState", equalTo("Succeeded"));
        given().when().get(BASE + "/containerGroups/cg-del" + API)
                .then().statusCode(404);
        // Spec: 204 = "Resource does not exist"; idempotent for an already-absent group.
        given().when().delete(BASE + "/containerGroups/cg-del" + API)
                .then().statusCode(204);
    }
