...
/**
 * Integration tests for the Azure Database for MariaDB (Single Server) emulation layer.
 *
 * <p>Requires:
 * <ul>
 *   <li>floci-az emulator running (default: http://localhost:4577)</li>
 *   <li>Docker available so floci-az can start MariaDB containers</li>
 * </ul>
 *
 * <p>Single Server uses {@code Microsoft.DBforMariaDB/servers} (not flexibleServers) and
 * api-version 2018-06-01 — the contract the azurerm provider's vendored go-azure-sdk
 * compiles against (spec: Microsoft.DBforMariaDB/stable/2018-06-01).
 *
 * <p>Run selectively (Docker needed):
 * <pre>
 *   mvn test -Dtest=MariaDbCompatibilityTest -pl compatibility-tests/sdk-test-java
 * </pre>
 */
@DisplayName("MariaDB Single Server compatibility")
...
class MariaDbCompatibilityTest {
    ...
    private static final String SERVER = "compat-mariadb-server";
    private static final String DB     = "compat_db";
    private static final String LOGIN  = "mariadbadmin";
    private static final String PWD    = "FlociAz_Strong123!";

    private static final String ARM_BASE =
        BASE + "/subscriptions/" + SUB + "/resourceGroups/" + RG + "/providers/Microsoft.DBforMariaDB";
    private static final String API = "?api-version=2018-06-01";

    // ── Shared state populated by @BeforeAll ──────────────────────────────────

    /** JDBC URL for the server's default {@code floci} database. */
    ...
    static void createServer() throws Exception {
        ...
        // Allow up to 5 minutes: first-time image pull (mariadb:10.11) + container start.
        HttpResponse<String> resp = send("PUT",
            ARM_BASE + "/servers/" + SERVER + API, serverBody, Duration.ofMinutes(5));
        ...
        HttpResponse<String> connectResp = send("GET",
            BASE + "/devstoreaccount1-mariadb/servers/" + SERVER + "/connect", null);
        ...
        // Derive the app-database URL from the default (.../floci?...  ->  .../compat_db?...).
        appJdbcUrl = defaultJdbcUrl.replace("/floci?", "/" + DB + "?");
        // Fail loudly if the replace did not match (e.g. the connection-string format changed),
        // rather than silently running the DDL/DML test against the default `floci` database.
        assertNotEquals(defaultJdbcUrl, appJdbcUrl,
            "app JDBC URL derivation failed — '/floci?' pattern not found in: " + defaultJdbcUrl);
    }
    ...
    static void deleteServer() throws Exception {
        if (http == null) return;
        try {
            send("DELETE", ARM_BASE + "/servers/" + SERVER + API, null);
    ...
    void getServer() throws Exception {
        HttpResponse<String> resp = send("GET",
            ARM_BASE + "/servers/" + SERVER + API, null);
        ...
        assertTrue(body.contains("\"userVisibleState\":\"Ready\""), "userVisibleState=Ready");
    ...
    void listServers() throws Exception {
        HttpResponse<String> resp = send("GET", ARM_BASE + "/servers" + API, null);
        assertEquals(200, resp.statusCode());
    ...
    @Test
    @Order(20)
    @DisplayName("JDBC connect to the default floci database succeeds")
        ...
        try (Connection conn = DriverManager.getConnection(defaultJdbcUrl)) {
            ...
            try (Statement st = conn.createStatement();
                ...
                assertTrue(version.toLowerCase().contains("mariadb"),
                    "VERSION() should mention MariaDB: " + version);
            }
        }
    }
    ...
    void createDatabase() throws Exception {
        String dbBody = "{\"properties\":{\"charset\":\"utf8\",\"collation\":\"utf8_general_ci\"}}";

        HttpResponse<String> resp = send("PUT",
            ARM_BASE + "/servers/" + SERVER + "/databases/" + DB + API,
    ...
    void checkNameUnavailable() throws Exception {
        String body = "{\"name\":\"" + SERVER + "\",\"type\":\"Microsoft.DBforMariaDB/servers\"}";
        // Single Server checkNameAvailability is subscription-scoped, with no locations segment
        // (spec: CheckNameAvailability_Execute, stable/2018-06-01).
        HttpResponse<String> resp = send("POST",
            BASE + "/subscriptions/" + SUB
            + "/providers/Microsoft.DBforMariaDB/checkNameAvailability" + API,
            body);
        assertEquals(200, resp.statusCode());
    ...
    void firewallRuleCrud() throws Exception {
        ...
        String ruleUrl = ARM_BASE + "/servers/" + SERVER + "/firewallRules/" + ruleName + API;
        ...
        HttpResponse<String> listResp = send("GET",
            ARM_BASE + "/servers/" + SERVER + "/firewallRules" + API, null);
    ...
    void configurationRoundTrip() throws Exception {
        String cfgUrl = ARM_BASE + "/servers/" + SERVER + "/configurations/max_connections" + API;
    ...
    void deleteDatabase() throws Exception {
        HttpResponse<String> resp = send("DELETE",
            ARM_BASE + "/servers/" + SERVER + "/databases/" + DB + API, null);
    ...
    @Test
    @Order(80)
    @DisplayName("_admin/reset wipes all MariaDB state and frees the server name")
    void adminResetClearsState() throws Exception {
        HttpResponse<String> before = send("GET", ARM_BASE + "/servers/" + SERVER + API, null);
        ...
        HttpResponse<String> afterServer = send("GET", ARM_BASE + "/servers/" + SERVER + API, null);
        assertEquals(404, afterServer.statusCode(), "Server should be gone after reset: " + afterServer.body());

        String checkBody = "{\"name\":\"" + SERVER + "\",\"type\":\"Microsoft.DBforMariaDB/servers\"}";
