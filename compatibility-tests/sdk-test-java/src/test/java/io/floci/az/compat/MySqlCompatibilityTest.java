...
/**
 * Integration tests for the Azure Database for MySQL (Flexible Server) emulation layer.
 *
 * <p>Requires:
 * <ul>
 *   <li>floci-az emulator running (default: http://localhost:4577)</li>
 *   <li>Docker available so floci-az can start MySQL containers</li>
 * </ul>
 *
 * <p>The api-version matches what the azurerm Terraform provider pins for
 * {@code Microsoft.DBforMySQL/flexibleServers} (spec: FlexibleServers/stable/2023-12-30).
 *
 * <p>Run selectively (Docker needed):
 * <pre>
 *   mvn test -Dtest=MySqlCompatibilityTest -pl compatibility-tests/sdk-test-java
 * </pre>
 */
@DisplayName("MySQL Flexible Server compatibility")
...
class MySqlCompatibilityTest {
    ...
    private static final String SERVER = "compat-mysql-server";
    private static final String DB     = "compat_db";
    private static final String LOGIN  = "mysqladmin";
    private static final String PWD    = "FlociAz_Strong123!";

    private static final String ARM_BASE =
        BASE + "/subscriptions/" + SUB + "/resourceGroups/" + RG + "/providers/Microsoft.DBforMySQL";
    private static final String API = "?api-version=2023-12-30";
    ...
    static void createServer() throws Exception {
        ...
        // Allow up to 5 minutes: first-time image pull (mysql:8.0) + container start.
    ...
    void jdbcConnectDefault() throws Exception {
        try (Connection conn = DriverManager.getConnection(defaultJdbcUrl)) {
            ...
            try (Statement st = conn.createStatement();
                ...
                // Guard against the wrong engine answering (MariaDB brands its version string).
                assertFalse(version.toLowerCase().contains("mariadb"),
                    "expected a MySQL server, got: " + version);
            }
        }
    }
    ...
    void createDatabase() throws Exception {
        String dbBody = "{\"properties\":{\"charset\":\"utf8mb4\",\"collation\":\"utf8mb4_general_ci\"}}";
    ...
    void checkNameUnavailable() throws Exception {
        String body = "{\"name\":\"" + SERVER + "\",\"type\":\"Microsoft.DBforMySQL/flexibleServers\"}";
        HttpResponse<String> resp = send("POST",
            BASE + "/subscriptions/" + SUB
            + "/providers/Microsoft.DBforMySQL/locations/eastus/checkNameAvailability" + API,
            body);
        assertEquals(200, resp.statusCode());
    ...
    @Test
    @Order(80)
    @DisplayName("_admin/reset wipes all MySQL state and frees the server name")
    void adminResetClearsState() throws Exception {
        ...
        String checkBody = "{\"name\":\"" + SERVER + "\",\"type\":\"Microsoft.DBforMySQL/flexibleServers\"}";
